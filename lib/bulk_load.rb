module BulkLoad
  ...
  module ClassMethods
    ...
    # * :silence => (true | false)
    #
    def bulk_load(records, keys, options = {})
      return if records.blank?
      keys = keys.respond_to?(:map) ? keys : [keys].compact
      options = DEFAULT_OPTIONS.merge(options)
      loader_class = self.const_get(options[:db_adaptor] + 'Loader')
      if options[:silence]
        self.silence {loader_class.new(self, options).bulk_load(records, keys)}
      else
        loader_class.new(self, options).bulk_load(records, keys)
      end
    end
...
UPDATE #{table_name}
 ...
 WHERE #{ keys.map {|k| "#{table_name}.#{k} = candidates.#{k}"}.join(' AND ') }
  ...
  # Support for MySQL2
  #
  # TODO: this is the same as MySQL.  What's the best way to alias?
  class Mysql2Loader < BaseLoader
